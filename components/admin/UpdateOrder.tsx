...
const UpdateOrder = ({ order }: { order: OrderProps }) => {
    const { updateOrder, error, clearErrors, updated, setUpdated } = useOrder();
    ...
    const submitHandler = () => {
        ...
        updateOrder(order?._id as string, orderData);
    }
                    ...
                    <figure key={index} className="flex flex-row mb-4">
                        ...
                        <figcaption className="ml-3">
                            ...
                            <p className="mt-1 font-semibold">
                                {item.quantity}x = ${Number(item.price) * Number(item.quantity)}
                            </p>
                        </figcaption>
                    </figure>
                ))}
